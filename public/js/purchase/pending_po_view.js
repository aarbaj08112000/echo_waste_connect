...
const page = {
    ...
    dataTable: function(){
        var data = {};
        table = $("#pending_po_view").DataTable({
        dom: "Bfrtilp",
        buttons: [
            {
                extend: "csv",
    ...
    initiateForm: function(){
        let that = this;
        $(".close_po_action").submit(function(e){
            var id = $(this).attr("data-id")
            e.preventDefault(); // Prevent form submission
            let flag = that.formValidate("close_po_action"+id);
            if(flag){
                return;
            }
           
                var formData = new FormData($('.close_po_action')[0]);
                $.ajax({
                    type: "POST",
