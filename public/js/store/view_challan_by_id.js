...
const page = {
  ...
  initiateForm: function(){
    ...
    $(".challan_return_qty").submit(function(e){
      e.preventDefault();
      let flag = that.formValidate("challan_return_qty");
      if(flag){
        return;
      }
      var formData = new FormData($('.challan_return_qty')[0]);

      $.ajax({
        type: "POST",
        // url: base_url+"add_challan_parts_history_challan",
        url: "add_challan_parts_history_challan",
        data: formData,
        processData: false,
        contentType: false,
    ...
    $(".challan_qty").submit(function(e){
      e.preventDefault();
      let flag = that.formValidate("challan_qty");
      if(flag){
        return;
      }
      var formData = new FormData($('.challan_qty')[0]);

      $.ajax({
        type: "POST",
        // url: base_url+"update_challan_parts_history_challan",
        url: "update_challan_parts_history_challan",
        data: formData,
        processData: false,
        contentType: false,
        success: function (response) {
