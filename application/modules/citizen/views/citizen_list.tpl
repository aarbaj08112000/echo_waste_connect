<div class="wrapper container-xxl flex-grow-1 container-p-y">
   ...
   <div class="dt-top-btn d-grid gap-2 d-md-flex justify-content-md-end mb-5">
      <button type="button" class="btn btn-seconday hide" data-bs-toggle="modal" data-bs-target="#addPromo">
      Add Citizen
      </button>
                     ...
                     <div class="table-responsive text-nowrap">
                        <table id="erp_users" class="table table-striped w-100">
                           ...
                           <tbody>
                              ...
                              <%foreach from=$citizen item=u %>
                              <tr>
                                 <%assign var='units' value=explode(",",$u['unit_ids'])%>
                                 <%assign var='groups_arr' value=explode(",",$u['groups'])%>
                                 <td class="hide"><%$i %></td>
                                 <td class="grid_view_warehouse_title_icon"><img src="<%$u['image'] %>" width="62" height="62" class="list-image ma_profile_image"></td>
                                 <td><%$u['name'] %></td>
                                 <td><%$u['email'] %></td>
                                 <td class="hide"><%$u['password'] %></td>
                                 <td class="status-text <%getStatusClass($u['status']) %>"><%$u['status'] %></td>
                                 <td><%defaultDateFormat($u['added_date'])%></td>
                                 <td><%getDefaultDateTime($u['updated_date'])%></td>
                                 <td><%display_no_character($u['updated_by'])%></td>
                                 <td>
                                    <a data-bs-toggle="modal" data-bs-target="#acceptReject<%$i%>" title="Change Status"><i class="ti ti-user-check"></i></a>
                                    <div class="modal fade" id="acceptReject<%$i%>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                       <div class="modal-dialog  modal-dialog-centered" role="document">
                                          <div class="modal-content">
                                             <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Approve Citizen</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                                </button>
                                             </div>
                                             <form action="<%base_url('citizen/citizen/updateCitizenStatus') %>" method="POST" enctype="multipart/form-data" id="update_citizen_status<%$i%>" class="update_citizen_status update_citizen_status<%$i%> custom-form">
                                                <div class="modal-body">
                                                  <div class="row">
                                                     <div class="form-group">
                                                        <input type="hidden" name="citizen_id" value="<%$u['citizen_id']%>">
                                                     </div>
                                                     <div class="form-group">
                                                        <label for="on click url">Approve Status<span class="text-danger">*</span></label> <br>
                                                        <select name="approve_status" class="form-control select2 required-input"  >
                                                           <option value="Approved" <%if $u['status'] eq 'Approved'%>selected<%/if%>>Approved</option>
                                                           <option value="Reject" <%if $u['status'] eq 'Reject'%>selected<%/if%>>Reject</option>
                                    ...
                                    <a data-bs-toggle="modal" data-bs-target="#updatePromo<%$i%>" class="hide"><i class="ti ti-edit"></i></a>
...
<script type="text/javascript">
   ...
   var module_name = "Citizen";
</script>
<script src="<%$base_url%>public/js/citizen/citizen_list.js"></script>